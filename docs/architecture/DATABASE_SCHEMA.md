# Database Schema — AgriTrace Carbon
...
enum CropType { RICE COFFEE PEPPER DURIAN VEGETABLE OTHER }
...
enum ActivityType { SEEDING FERTILIZING PESTICIDE IRRIGATION HARVESTING OTHER }
...
## Indexes quan trọng (ngoài PK)

```sql
-- Tìm kiếm nhanh QR
CREATE INDEX qr_code_code_idx ON "QrCode" (code);

-- Spatial index cho GPS overlap check
CREATE INDEX farm_zone_boundary_gist ON "FarmZone" USING GIST (boundary);

-- Notification
CREATE INDEX notif_user_read_idx ON "Notification" (user_id, is_read);

-- Farming log theo season
CREATE INDEX flog_season_type_idx ON "FarmingLog" (season_id, activity_type);
```
