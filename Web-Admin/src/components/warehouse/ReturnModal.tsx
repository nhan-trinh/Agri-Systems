...
interface ReturnModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: ReturnPayload) => Promise<void>;
  materials: Material[];
  farmers: Farmer[];
...
export function ReturnModal({
  ...
  const [formData, setFormData] = useState({
    material_id: '',
    quantity: '',
    recipient_farmer_id: '', // Using recipient_farmer_id to align with DB transaction schema
    purpose: '', // Used as "reason" in the backend transaction payload
    transaction_date: '',
    notes: '',
  });
  const [error, setError] = useState('');

  // Filter active materials for return
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.material_id) {
      setError('Vui lòng chọn vật tư hoàn trả');
      return;
    }
    ...
    if (!formData.transaction_date) {
      setError('Vui lòng chọn ngày hoàn trả');
      return;
    }
    ...
    try {
      const payload = {
        ...
        quantity: qtyVal,
        recipient_farmer_id: formData.recipient_farmer_id || undefined, // Optional farmer who returned it
        return_reason: formData.purpose.trim(),
      ...
      setError(errorResponse.response?.data?.message || 'Có lỗi xảy ra khi hoàn trả vật tư');
    }
  };
    ...
    <div className="fixed inset-0 bg-stone-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-xl border border-[#e6ebe3] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="flex justify-between items-center px-6 py-5 border-b border-[#e6ebe3] bg-[#fbfcf9]">
          <h3 className="font-serif text-lg font-bold text-[#1b4332]">Phiếu Hoàn Trả Vật Tư</h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            <label className="block text-xs font-bold text-stone-500 uppercase tracking-wider mb-2">
              Vật Tư Hoàn Trả <span className="text-red-500">*</span>
            </label>
            <select
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-stone-500 uppercase tracking-wider mb-2">
                Số Lượng Hoàn Trả <span className="text-red-500">*</span>
              </label>
            ...
            <div>
              ...
              >
                <option value="">-- Chọn nông dân hoàn trả --</option>
                {farmers.map((f) => (
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="VD: Vật tư dư sau vụ mùa, chất lượng tốt"
          ...
          <div>
            <label className="block text-xs font-bold text-stone-500 uppercase tracking-wider mb-2">
              Ngày Hoàn Trả <span className="text-red-500">*</span>
            </label>
            <input
              type="date"
